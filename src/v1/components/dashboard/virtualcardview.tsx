...
export default function VirtualCardView() {
    ...
    const handleRequest = async (): Promise<void> => {
        try {
            ...
            const res = await fetch(`${Defaults.API_BASE_URL}/user/virtual-card/request`, {
                method: 'POST',
                headers: {
                    ...Defaults.HEADERS,
            ...
            if (data.status === Status.SUCCESS) {
                ...
                if (userdata.status === Status.SUCCESS) {
                    ...
                    toast.success("Virtual Card request sent. We'll review your request and get back to you shortly.")
                }
            }
        } catch (error: any) {
            toast.error(error.message || "Error Activating Virtual Card");
        } finally {
            setLoading(false)
        }
    }

    if (user && user.requested && user.requested.virtualCard === RequestStatus.PENDING) {
        return (
            <div className="w-full mt-40 flex flex-col items-center justify-center">
                <div className="relative flex items-center justify-center w-20 h-20">
                    ...
                    />
                    <CreditCard size={28} className="relative z-10 text-yellow-900" />
                </div>
                <div className="flex flex-col items-center text-center justify-center gap-2 mt-3">
                    <h2 className="font-bold">Virtual Card Request Pending</h2>
                    <p>
                        Your request to activate your <span className="font-semibold">Virtual Card</span> is currently <span className="text-yellow-600 font-medium">pending approval</span>.<br />
                        You will be notified once your Virtual Card is enabled for your account.<br />
    ...
    return (
        <div className="w-full mt-40 flex flex-col items-center justify-center">
            <div className="relative flex items-center justify-center w-20 h-20">
                ...
                <CreditCard size={28} className="relative z-10 text-blue-900" />
            </div>

            <div className="flex flex-col items-center text-center justify-center gap-2 mt-3">
                <h2 className="font-bold">Activate Virtual Card</h2>
                <p>
                    To activate your Virtual Card, click on the
                    <span className="text-blue-500 font-medium">"Activate Virtual Card"</span> button below.<br />
                    This will send a request to our support team to enable Virtual Card for your account.
                </p>
                    ...
                    >
                        {loading ? "Loading..." : (
                            <>
                                ...
                                <span className="hidden sm:inline">Activate Virtual Card</span>
                            </>
                        )}
                    </Button>
                </div>
